...
- **Faz 4 — tray/dialog theme + focus + contrast:** the tray command palette (the only custom-drawn tray surface; tray icons/tooltips/balloons are OS-native) now reads `theme::tokens()` per paint for every color and redraws on `WM_SETTINGCHANGE`/`WM_DWMCOLORIZATIONCOLORCHANGED`, so an open palette follows a live theme flip. `ui::window_loop` centrally invalidates the theme cache on `WM_SETTINGCHANGE` and the accent cache on `WM_DWMCOLORIZATIONCOLORCHANGED` before dispatching, so every pumped window repaints fresh. The settings button paint now prioritizes focus: keyboard focus draws an accent ring (on-accent ring for the accent-filled Save button, which previously blended into its own fill), including unchecked radios/checkboxes/toggles since all owner-drawn controls share that paint path. Contrast measured (WCAG formula): light 17.2:1 body, 6.6:1 secondary, 6.3:1 on-accent; dark 14.2:1 body, 9.1:1 secondary, 10.5:1 on-accent; danger 5.7/7.0:1 — all pass 4.5:1; disabled text (2.6:1) is intentionally exempt.

- **Faz 5 — verification:** all gates (`fmt --check`, `check --all-targets --locked`, `clippy --all-targets -- -D warnings`, `build --release --locked`) passed after each phase commit; unit tests/smoke remain skipped per the user's direction. Runtime smoke on a real desktop: the fresh release binary opened Settings at exactly 800x600 at 96 DPI with the full Fluent layout, accent focus ring and working vertical scroll, then closed cleanly. Mica compositing, live dark-mode flips, the tray palette at 125-200% DPI and the toolbar icons on-canvas belong to the user's visual acceptance pass. An earlier staged commit (`087799d`) briefly contained a non-compiling `std::str::from_utf16` call, fixed by `fe90055` before any later phase built on it.
